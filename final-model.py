...
def load_frames(frames_dir, target_num_frames, video_length):
    print("Loading " + frames_dir)
    frames = []
    starting_frame = 0
    directory = sorted(os.listdir(frames_dir))

    if video_length > target_num_frames:
        starting_frame = (video_length-target_num_frames)/2

    for i in range(int(starting_frame), len(directory)):
        img = cv2.imread(os.path.join(frames_dir, directory[i]), cv2.IMREAD_GRAYSCALE)
    ...
    while len(frames) < target_num_frames:
        frames.append(np.zeros((64 * 64), dtype=np.float32).flatten())
...
def count_frames(frames_dir):
    ...
    print(f'Video {frames_dir} has {num_frames} frames')
    return num_frames
...
for video_folder in sorted(os.listdir(videos_dir)):
    ...
    length = count_frames(video_frames_path)
    video_features = load_frames(video_frames_path, target_num_frames, length)
...
# print(f'Average number of frames per video is {np.mean(np.array(video_lengths))}')
